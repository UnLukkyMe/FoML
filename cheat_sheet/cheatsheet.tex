...
\begin{multicols}{5}
    ...
    \textbf{Majority Class} $Maj_D(M')$ maj. $r\in R$

    \textbf{Number of misclassifications}: $Err_D(M', r)$ in feature subspace $M'$ with majority class $r$

    - $Err_D(T)$: sum up all $Err_D(M_v, T(v))$

    \textbf{pure node} v if $Err_D(M_v, T(v))=0$

    - class distribution $p_D^{M'}(r)$: $p(r)$ in $M_v$

    \textbf{impurity function}
