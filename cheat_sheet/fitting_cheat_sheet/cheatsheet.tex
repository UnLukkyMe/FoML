...
\begin{multicols}{5}
    ...
    $h_w^H(z)=H(\sum_{i=0}^{k}w_iz_i)=$ 1 if $wz^T\geq 0$
        ...
        \end{enumerate}
        Rosenblatt: If D lnrly seprble: PT terminates after finitely many corrections
        \begin{center}
    ...
    $h_W^v(z): \mathbb{R}' \to \mathbb{R}, h_W^v(z)=s_v^{(m)}$
        ...
        - $(s_v - y)\cdot H(r_v) $ if $v$ designated o node

        - 0 if $v$ other o node

        - $H(r_v)\cdot \sum_{v'\in V} W_{vv'}\cdot \frac{\partial L}{\partial r_{v'}}$ otherwise
    ...
    $h: M \to \{1, ..., c\}$ assigns each point a \textbf{cluster}: names (1, ..., c) arbitrary

        - loss is avg of squared euclidian distance to cluster mean (centroid)

        \textbf{c-means (Lloyds) Algorithm}
        \begin{enumerate}
            \item init c centroids $m_1, ..., m_c$ random
            \item Add each $i$ of $x_i$ to closest centroids ($m_j$) Set $I_j$
            \item Update centroids to mean of assigned points. $m_j= \frac{1}{n}\sum_{i\in I_j}
                      x_i$
        \end{enumerate}
        - induced hypothesis: $h_{m_1, ..., m_c}(x)=\arg \min_{1\leq j \leq c} ||x-m_j||^2$
        ...
        \begin{enumerate}
            ...
            \item greedily merge minimal scoring clusters to new clusters
            \item Stop combination once cluster-cluster score > threshold $\theta$
        \end{enumerate}
        ...
        - Trace: starts in initial state, respects $\Delta$, ends in terminal state: $q_0 \rightarrow^{a_1} q_1...$

        - Rewards: $R: \Delta \to \mathbb{R}$, reward for taking transition (mostly only for terminal transitions: win/lose)

        - Policy: assigns propability to each action in each state: $P: Q \times \Sigma \to [0,1]$

    $P(a | q)$ determines Agents action

    $P$ detm. if $\forall q \in Q: \exists a \in \Sigma: P(a | q)=1$

        - \textbf{Bellman equation} - estimate value of States: to steer towards high-value states

    $V(q) =\max_{a\in \Sigma} \sum_{q' \in Q} P(q'|q,a) \cdot (R(q,a,q') + \gamma V(q'))$: 'likelyhood of transisiton $\delta \cdot$ direct reward + discounted future reward' (Q-Value $Q_V(q, a)$ is the same $V(q)$ but for specific action $a$)

        - $\gamma$ discount factor: future rewards discounted by $\gamma^n$ for n steps in future

        \textbf{Value iteration}: for every state compute $V(q)$ until convergence (finds optimal Q-values $V^*$)

        given $V^*$: optimal policy selects action with highest expected reward:

        \textbf{Policy iteration}

        -Q-Value with deterministic policy $P$: $Q_V^P(q)=\sum_{q'\in Q}p(q'|q, P(q)) \cdot (R(q, P(q), q') + \gamma V(q'))$

        1. for every state compute $Q_V^P(q)$ until convergence

        2. update policy to take optimal action

        3. If $\lnot$converged $\to $1

        - both methods are model based: they have perfect information (exploitation)

        \textbf{No perfect information: model free}
...
\begin{multicols}{2}
    ...
    \begin{itemize}
        \item \textbf{(0-1 loss)} $l_{0/1}(h, D)=\sum_{i=1}^{n}(1-\delta_{y_i, h(x_i)})$\\
              ->  $\delta_{ij} =1 \text{ if } i = j,0  \text{ otherwise.}$
        \item \textbf{(Mean Squared Error)} $l_2(h, D)=\frac{1}{2n}\sum_{i=1}^{n}( h(x_i) - y_i)^2$ \\
              $\frac{\partial l(h, D)}{\partial w_p}=\frac{1}{n}\sum_{i=1}^{n}(h(x_i) - y_i)x_p=\frac{1}{n}\sum_{i=1}^{n}(w_0+w_1x_{i1}+...+w_p x_{ip}-y_i)x_p$
        \item \textbf{(Ridge Regression)} $l'(w, D)=\frac{1}{2n}\sum_{i=1}^{n}( h(x_i) - y_i)^2 + \frac{\lambda}{k}\sum_{i=1}^{k}w_i^2$\\
              $\frac{\partial l'(w, D)}{\partial w_p}=\frac{1}{n}\sum_{i=1}^{n}(h(x_i) - y_i)x_p+ 2\lambda w_p, \ w_p \in \{0, w_1,... w_k\}$
        \item \textbf{(Logistic Loss)}       $\ell_\sigma(h, D) = -\frac{1}{n} \sum_{i=1}^{n} \left(          y_i \cdot \log\bigl(h(x_i)\bigr)          + (1 - y_i) \cdot \log\bigl(1 - h(x_i)\bigr)          \right)$\\
              $\frac{\partial \ell_\sigma(h_w^\sigma, D)}{\partial w_p} = -\frac{1}{n} \sum_{i=1}^{n}\left( y_i - h_w^\sigma(x_i) \right)\cdot x_{ip}$
        \item \textbf{(Hinge Loss)} $l_h(h, D)=\frac{1}{n}\sum_{i=1}^{n}\max (0, 1-(2y_i-1)h(x_i))$
        \item  \textbf{(Cross Entropy Loss)} $l_H(h, D)=\frac{1}{n}\sum_{i=1}^{n}H(1_{y_i}, h(x_i))$  \\
              -> $H(1_y, p)=-\sum_{r \in R} 1_y(r) \log p(r)=-\log p(y)$\\
              $\frac{\partial H(1_y, p)}{\partial s_v}=\frac{p(v)-1_y(v)}{\ln 2}$
        \item  \textbf{(average within-cluster variance)} $l_V(h, D)=\frac{1}{n}\sum_{i=1}^{n}||x_i-\mu_i||^2$ where $\mu_i$ is centroid of cluster that $x_i$ belongs to (euclidean distance to cluster mean)
    \end{itemize}
